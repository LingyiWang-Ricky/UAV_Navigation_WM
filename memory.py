...
class ExperienceReplay:
    def __init__(self, size, symbolic_env, observation_size, action_size, bit_depth, device):
        ...
        if isinstance(observation_size, dict):
            ...
            self.observations = {
                ...
                'target_position': np.empty((size, 2), dtype=np.float32),
                # [论文公式 8-9] 语义地图 M_t ∈ R^{6×Ng×Ng}, 值域 [-1, 1]
                # 存为 float16 节省一半内存 (100K帧: 1GB vs 2GB), 取出时转 float32
    ...
    def append(self, observation, action, reward, done):
        if isinstance(self.observations, dict):
            ...
            self.observations['target_position'][self.idx] = obs_np['target_position']
            # 语义地图: float32 [-1, 1], 不做 bit-depth 处理
    ...
    def _retrieve_batch(self, idxs, n, L):
        ...
        if isinstance(self.observations, dict):
            ...
            # target_position
            tgt_pos_data = self.observations['target_position'][vec_idxs]
            obs_batch['target_position'] = torch.as_tensor(tgt_pos_data).reshape(L, n, -1)
