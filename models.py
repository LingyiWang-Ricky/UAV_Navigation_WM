...
class MapTransitionModel(nn.Module):
    ...
    def forward(self, prev_map, belief, state, action, map_embedding):
        ...
        # 残差连接 + tanh 限制到 [-1, 1]
        next_map = torch.tanh(self.conv_out(h) + prev_map)
...
class UAVHybridEncoder(nn.Module):
    ...
    Siamese 结构共享权重, 分别处理当前图像和目标图像.
    输出:
        - embed: (B, embedding_size) — 融合向量 e_t
        - curr_emb, targ_emb: 各 (B, 1024) — Siamese 特征 (用于对比损失)
    """
    ...
    def forward(self, image, target_image, vector):
...
class TransitionModel(nn.Module):
    ...
    关键修改:
    - GRU 输入拼接了 m_{t-1} (地图嵌入), 对应公式14
    - posterior 拼接了 semantic_state, 对应公式16
    - 想象阶段 semantic_features=None 时使用 fc_embed_semantic 自回归
    """
    ...
    def __init__(
        ...
        # 论文: 拼接 belief + encoder_embedding + map_embedding
        # semantic_state 保留用于 System 2, 但不参与 posterior
        self.fc_embed_belief_posterior = nn.Linear(
            belief_size + embedding_size + map_embedding_size, hidden_size
        )
        self.fc_state_posterior = nn.Linear(hidden_size, 2 * state_size)

        # --- System 2 (Semantic Logic) ---
        self.semantic_rnn = nn.GRUCell(semantic_size, semantic_state_size)
        # 自回归回退 (想象阶段)
        self.fc_embed_semantic = nn.Linear(semantic_state_size, semantic_size)
    ...
    def forward(
            ...
            prev_semantic_state: torch.Tensor,
        ...
        Args:
            ...
            prev_semantic_state: (B, semantic_state_size)
        ...
        semantic_states[0] = prev_semantic_state
        ...
        for t in range(T - 1):
            ...
            # --- System 2: Semantic RNN ---
            t_ = t
            if semantic_features is not None:
                current_sem_input = semantic_features[t_]
            else:
                current_sem_input = self.fc_embed_semantic(semantic_states[t])

            semantic_states[t + 1] = self.semantic_rnn(current_sem_input, semantic_states[t])
        ...
        return (
            ...
            torch.stack(semantic_states[1:], dim=0)
        )
